    ...
    App::new()
        .add_plugins(DefaultPlugins.set(WindowPlugin {
            primary_window: Some(Window {
                title: "Runetika - Room Test (Fixed)".into(),
        ...
        .add_systems(Update, (movement, debug_info))
        .run();
}
...
fn setup(mut commands: Commands) {
    ...
    // Player (blue dot) - with Player component
    commands.spawn((
        Player,
        Sprite {
...
fn movement(
    ...
    for mut velocity in &mut query {
        ...
        if keyboard.pressed(KeyCode::KeyW) || keyboard.pressed(KeyCode::ArrowUp) {
            direction.y += 1.0;
            println!("W/Up pressed");
        }
        if keyboard.pressed(KeyCode::KeyS) || keyboard.pressed(KeyCode::ArrowDown) {
            direction.y -= 1.0;
            println!("S/Down pressed");
        }
        if keyboard.pressed(KeyCode::KeyA) || keyboard.pressed(KeyCode::ArrowLeft) {
            direction.x -= 1.0;
            println!("A/Left pressed");
        }
        if keyboard.pressed(KeyCode::KeyD) || keyboard.pressed(KeyCode::ArrowRight) {
            direction.x += 1.0;
            println!("D/Right pressed");
        }
...
fn debug_info(
    ...
    for mut transform in &mut query {
        ...
        if keyboard.get_pressed().len() > 0 {
            println!("Keys pressed: {:?}", keyboard.get_pressed().collect::<Vec<_>>());
        }
    }
}