...
fn process_terminal_input(
    ...
    for event in events.read() {
        ...
        match &event.logical_key {
            Key::Character(c) => {
                ...
                terminal_state.current_input.insert_str(pos, &c_str);
                terminal_state.cursor_position += c_str.len();
            }
            Key::Space => {
                let pos = terminal_state.cursor_position;
                terminal_state.current_input.insert(pos, ' ');
    ...
    if keyboard.just_pressed(KeyCode::Enter) && !terminal_state.current_input.is_empty() {
        // Add to output
        let input_text = format!("> {}", terminal_state.current_input);
    ...
    if keyboard.just_pressed(KeyCode::Backspace) && terminal_state.cursor_position > 0 {
        terminal_state.cursor_position -= 1;
        let pos = terminal_state.cursor_position;
        terminal_state.current_input.remove(pos);
    }
